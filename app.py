...
# Background parsers and synchronizers run by default. Set
# ENABLE_BACKGROUND_JOBS=0 only for an intentional read-only diagnostic run.
_ENABLE_BACKGROUND_JOBS = os.environ.get(
    'ENABLE_BACKGROUND_JOBS',
    '1',
).strip().lower() in {'1', 'true', 'yes', 'on'}

def _start_background_thread(target, *, name=None):
    if not _ENABLE_BACKGROUND_JOBS:
        return None
...
if not _ENABLE_BACKGROUND_JOBS:
    logger.info('Фоновые парсеры и синхронизаторы отключены: ENABLE_BACKGROUND_JOBS не включён')
...
_COUNTED_CATEGORIES = (
    'real_estate', 'transport', 'restaurants', 'tours', 'entertainment',
    'money_exchange', 'visas', 'marketplace',
)
_COUNT_TRUSTED_ENTERTAINMENT_SOURCES = {
    'gavisarun', 'gatours', 'gafoods', 'gapayments',
    'tusaparsing_vn', 'tusaparsing_th', 'tusaparsing_in', 'tusaparsing_indo',
    'excursii_th',
}
_COUNT_ENTERTAINMENT_KEYWORDS = (
...
_COUNT_TRANSPORT_SPAM_KEYWORDS = (
    'продажа готового бизнеса', 'apple watch', 'освободился номер',
    'освободились апартаменты', 'медицинское страхование',
    'верификации аккаунта казино', '/app rentgo', 'казино за',
    'нужны люди для', 'двухкомнатная студия', 'сравнение цен',
    'цены для сравнения', 'цены для сравнение', 'vnd.день', 'vnd.day',
    'для управления этим байком не требуются права',
    'доступно для аренды сегодня', 'уважаемые подписчики',
    'не знаете, чем заняться', 'внимание внимание',
    'апартаменты с джакузи', 'для управления этого байка не требуются',
    'доступен дешевый велосипед', '600.vnd',
    'обучение езде на мотобайке', 'доступен шоссейный велосипед',
    'свободен для аренды мотоцикл', 'honda win100',
    'у тебя день рождения', 'лови скидку',
)

def _public_items_for_counter(country, category, data):
    """Базовые фильтры публичной выдачи без пользовательских параметров."""
    seen_ids = set()
    items = []
    ...
    if category == 'entertainment':
        def _is_public_entertainment(item):
            if item.get('_telegram_active') is not True:
                return False
            source_values = (
                item.get('source_group', ''), item.get('source_channel', ''),
                item.get('channel', ''),
            )
            trusted_source = any(
                str(source).lower() in _COUNT_TRUSTED_ENTERTAINMENT_SOURCES
                for source in source_values
            )
            text = ' '.join(str(item.get(key, '') or '') for key in ('description', 'title', 'text')).lower()
            has_keyword = any(keyword in text for keyword in _COUNT_ENTERTAINMENT_KEYWORDS)
            photos = item.get('photos') or item.get('all_images') or []
            has_photo = bool(photos or item.get('image_url') or item.get('photo') or item.get('photo_url'))
            return has_photo and (trusted_source or has_keyword)
        return [item for item in items if _is_public_entertainment(item)]

    if category == 'tours' and country == 'vietnam':
        return [item for item in items if item.get('source_group') == 'GAtours_vn']

    if category == 'real_estate':
        own_contacts = {'@chatparsing_vn', '@tusaparsing_vn'}
        return [
            item for item in items
            if item.get('contact', '').lower() not in own_contacts
            and item.get('image_url') and str(item.get('image_url')).strip()
        ]

    if category == 'transport':
        def _is_transport_spam(item):
            text = ' '.join(str(item.get(key, '') or '') for key in ('title', 'description', 'text')).lower()
            return any(keyword in text for keyword in _COUNT_TRANSPORT_SPAM_KEYWORDS)
        return [item for item in items if not _is_transport_spam(item)]

    return items

def _public_listing_counts(country, data):
    return {
        category: len(_public_items_for_counter(country, category, data))
        for category in _COUNTED_CATEGORIES
    }
...
def api_counts():
    ...
    try:
        ...
        counts = _public_listing_counts(country, data)
        resp = fast_json(counts)
        # Счётчики меняются вместе с фоновым парсером: браузер не должен
        # показывать сохранённый ноль после появления или очистки объявлений.
...
def _invalidate_listing_caches(country=None):
    """Сбрасывает все кэши, связанные с объявлениями.

    Фоновые сборщики пишут country JSON напрямую, поэтому одной очистки
    data_cache недостаточно: старые ответы могли оставаться в кэше списка и
    объединённого /api/init ещё несколько минут.
    """
    if country:
        data_cache.pop(country, None)
        _init_cache.pop(country, None)
        _logo_fps_cache.pop(country, None)
        marker = f':{country}:'
        for key in [key for key in _filtered_cache if marker in key]:
            _filtered_cache.pop(key, None)
    else:
        data_cache.clear()
        _init_cache.clear()
        _filtered_cache.clear()
        _logo_fps_cache.clear()
...
def api_init():
    ...
    counts = _public_listing_counts(country, data)
    total_listings = sum(
        len(_public_items_for_counter(country, category, data))
...
def status():
    ...
    for category in _COUNTED_CATEGORIES:
        public_categories.setdefault(category, 0)
    total_items = sum(public_categories.values())
    total_listings = sum(
        count for category, count in public_categories.items()
        if category != 'chat'
    )
...
def get_city_counts(category):
    ...
    # Дальше ответ дополняется и маскируется для клиента. Работаем с копиями,
    # чтобы не потерять source_channel/message_id в серверном кэше: эти поля
    # нужны последующим запросам для проверки связи карточки с Telegram-группой.
    listings = [dict(item) for item in _public_items_for_counter(country, category, data)]
...
_STRIP_FIELDS = frozenset({
    'bot_msg_id', 'city_ru', 'file_id', 'has_media', 'media_group_id',
    'source_group', 'group_message_id', '_telegram_active',
})
...
def get_listings(category):
    ...
    # Развлечения синхронизируются с Telegram: здесь важнее свежесть, чем
    # кэширование. Иначе браузер может показать удалённую карточку до TTL.
    _use_response_cache = not _admin_req and category != 'entertainment'
    ...
    # В развлечениях показываем только карточки, подтверждённые в Telegram-группе.
    # Флаг хранится отдельно от source_channel, который скрывается в ответе.
    if category == 'entertainment':
        filtered = [item for item in filtered if item.get('_telegram_active') is True]
    ...
    if category == 'transport':
        ...
        # Подготовка ответа не должна менять объекты в data_cache: внутренние
        # Telegram-маркеры нужны счётчикам и следующему запросу.
    ...
    # API-маскировка применяется только к копиям, а не к серверному кэшу.
    filtered = [dict(item) for item in filtered]
    _enrich_tg_images(filtered)
    _mask_internal_channels(filtered)
    _strip_api_payload(filtered)
...
_start_background_thread(_sync_media_vn_banners, name='BannerMediaVnSync')
_start_background_thread(_banner_refresh_scheduler, name='BannerRefreshScheduler')
...
_start_background_thread(_prewarm_banner_video_cache, name='BannerPrewarm')
...
_start_background_thread(_banner_vn_cleanup, name='BannerVnCleanup')
logger.info('[banner_cleanup] Авто-очистка баннеров Вьетнам запущена (каждые 5 мин)')
...
_start_background_thread(_prewarm_restaurant_file_paths)
...
_start_background_thread(_prewarm_restaurant_disk_photos, name='DiskPhotoPrewarm')
...
def admin_migrate_photos():
    ...
    for fname, country in _LIST_FILES:
        ...
        if changed:
            ...
            # Сбрасываем кэш
            try:
                _invalidate_listing_caches(country)
            except Exception:
                pass
...
_start_background_thread(_build_reverse_file_id_index, name='ReverseFileIdIndexer')


_start_background_thread(_build_msg_to_file_id_index, name='FileIdIndexer')
...
_start_background_thread(_file_id_autosave_loop, name='FileIdAutosave')


def _auto_set_webhook():
    """Автоматически устанавливает webhook бота при запуске.
    Приоритет: WEBHOOK_URL → WEBAPP_URL → SPACE_HOST (HF Spaces) → REPLIT_DOMAINS → REPLIT_DEV_DOMAIN"""
    import time as _time
    _time.sleep(5)
    try:
        bot_token = os.environ.get('TELEGRAM_BOT_TOKEN', '').strip()
...
# Webhook автосет: запускается на HF Space, опубликованном Replit-приложении или при FORCE_WEBHOOK=1
_has_host = os.environ.get('SPACE_HOST') or os.environ.get('WEBAPP_URL') or os.environ.get('REPLIT_DOMAINS') or os.environ.get('REPLIT_DEV_DOMAIN') or os.environ.get('FORCE_WEBHOOK') == '1'
if _has_host:
    _start_background_thread(_auto_set_webhook, name='BotWebhookSet')
else:
    logger.info('[bot] Webhook autoset отключён — не найден SPACE_HOST / REPLIT_DOMAINS. Задайте FORCE_WEBHOOK=1 для принудительного запуска')
...
def _process_routed_channel_post(cp):
    ...
    try:
        ...
        if category_r == 'entertainment' and chat_username in {
            'tusaparsing_vn', 'tusaparsing_th', 'tusaparsing_in', 'tusaparsing_indo',
        }:
            item_r['_telegram_active'] = True
            item_r['group_message_id'] = msg_id
        ...
        if added:
            _invalidate_listing_caches(country_r)
...
_start_background_thread(_gavibeshub_poller, name='GAvibeshubPoller')
...
def _scrape_channel_latest(channel, category, target_file, country):
    ...
    try:
        ...
        for msg_id in sorted(scraped.keys(), reverse=True):
            ...
            if category == 'entertainment' and channel in {
                'tusaparsing_vn', 'tusaparsing_th', 'tusaparsing_in', 'tusaparsing_indo',
            }:
                new_item['_telegram_active'] = True
                new_item['group_message_id'] = msg_id
...
_start_background_thread(_warmup_cache, name='CacheWarmup')
_start_background_thread(_all_channels_periodic_scraper, name='AllChannelsScraper')
_start_background_thread(_chat_periodic_scraper, name='ChatScraper')
_start_background_thread(_sync_restoranparsing_all, name='RestoranSync')
_start_background_thread(_startup_backfill, name='StartupBackfill')
...
_start_background_thread(_sync_github_tours, name='GithubToursSync')
...
_start_background_thread(_hf_auto_sync, name='HfAutoSync')
...
_start_background_thread(_partyhunt_poller, name='PartyHuntPoller')
...
_start_background_thread(_sync_entertainment_banners_th_indo, name='EntBannerSyncThIndo')
...
import threading
_start_background_thread(_bg_chatiparsing_poller, name='ChatiparsingPoller')
...
_start_background_thread(_india_entertainment_cleanup, name='IndiaEntCleanup')
logger.info('[india_ent_cleanup] Auto-cleanup started (every 6h, window=14d)')


_ENTERTAINMENT_TELEGRAM_SOURCES = (
    ('tusaparsing_vn', 'listings_vietnam.json'),
    ('tusaparsing_th', 'listings_thailand.json'),
    ('tusaparsing_indo', 'listings_indonesia.json'),
    ('tusaparsing_in', 'listings_india.json'),
)
_ENTERTAINMENT_CLEANUP_HEADERS = {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
    'Accept-Language': 'ru,en;q=0.9',
}


def _entertainment_listing_message_id(item, channel):
    """Возвращает ID поста именно в группе-источнике, а не ID пересланного оригинала."""
    group_message_id = item.get('group_message_id') or item.get('bot_msg_id')
    if str(group_message_id).isdigit():
        return int(group_message_id)
    message_id = item.get('message_id')
    if str(message_id).isdigit():
        return int(message_id)
    match = re.search(rf'{re.escape(channel)}_(\d+)$', str(item.get('id', '')))
    return int(match.group(1)) if match else None


def _collect_public_channel_ids(channel, min_message_id):
    """Собирает публичные ID до нужной глубины.

    Возвращает None при ошибке доступа, чтобы временная ошибка Telegram никогда
    не стала причиной удаления актуальных карточек.
    """
    import time as _time
    ...
    for _page in range(50):
        try:
            response = _requests.get(
                ...
                headers=_ENTERTAINMENT_CLEANUP_HEADERS,
                timeout=20,
            )
            if response.status_code != 200:
                logger.warning('[ent_cleanup] @%s returned HTTP %s', channel, response.status_code)
                return None
        except Exception as exc:
            logger.warning('[ent_cleanup] @%s scrape error: %s', channel, exc)
            return None

        page_ids = {int(value) for value in re.findall(r'data-post="[^/"]+/(\d+)"', response.text)}
        if not page_ids:
            # Пустая страница после успешного ответа значит, что более старых
            # постов нет. Пустая первая страница — действительно пустой канал.
            return live_ids

        live_ids.update(page_ids)
        oldest_id = min(page_ids)
        if oldest_id <= min_message_id:
            return live_ids
        before = oldest_id
        _time.sleep(1)

    logger.warning('[ent_cleanup] @%s: reached pagination limit before message %s', channel, min_message_id)
    return None


def _cleanup_entertainment_source(channel, filename):
    """Оставляет только текущие сообщения заданной Telegram-группы."""
    ...
    try:
        ...
        for item in entertainment:
            source_channel = (item.get('source_channel') or item.get('channel') or '').lstrip('@').lower()
            if source_channel == channel:
                source_items.append((item, _entertainment_listing_message_id(item, channel)))
        verifiable = [(item, message_id) for item, message_id in source_items if message_id is not None]
        live_ids = set()
        if verifiable:
            live_ids = _collect_public_channel_ids(channel, min(message_id for _, message_id in verifiable))
            if live_ids is None:
                return 0

        kept = []
        changed = len(entertainment) != len(verifiable)
        for item, message_id in verifiable:
            if message_id in live_ids:
                if item.get('_telegram_active') is not True or item.get('group_message_id') != message_id:
                    changed = True
                item['_telegram_active'] = True
                item['group_message_id'] = message_id
                kept.append(item)

        removed_count = len(entertainment) - len(kept)
        changed = changed or removed_count > 0
        if not changed:
            logger.info('[ent_cleanup] @%s: all %d listings are current', channel, len(kept))
            return 0

        data['entertainment'] = kept
        ...
        _filtered_cache.clear()
        logger.info(
            '[ent_cleanup] @%s: removed %d non-current listings; %d entries remain',
            channel, removed_count, len(data['entertainment']),
        )
        return removed_count
    except Exception as exc:
        logger.warning('[ent_cleanup] @%s error: %s', channel, exc)
        return 0


def _entertainment_telegram_cleanup():
    """Сверяет все группы развлечений с Telegram сразу после запуска и каждые 4 часа."""
    import time as _time

    while True:
        removed = sum(
            _cleanup_entertainment_source(channel, filename)
            for channel, filename in _ENTERTAINMENT_TELEGRAM_SOURCES
        )
        if removed:
            logger.info('[ent_cleanup] Total unavailable entertainment listings removed: %d', removed)
        _time.sleep(4 * 3600)


_start_background_thread(_entertainment_telegram_cleanup, name='EntertainmentTelegramCleanup')
logger.info('[ent_cleanup] Telegram cleanup for entertainment groups started (every 4h)')
...
_rates_thread = _start_background_thread(update_paymens_rates, name='RatesUpdater')
...
_start_background_thread(_sc_daily_refresh_loop, name='SCDailyRefresh')
...
_start_background_thread(_dz_daily_refresh_loop, name='DzDailyRefresh')
