...
- Use the **Run** button to launch the `Start application` workflow and open the dashboard preview.
- The dashboard and bundled listing data work without additional credentials. Telegram-powered parsing, forwarding, and file retrieval are optional features that need the relevant Telegram secrets (`TELEGRAM_BOT_TOKEN`, `VIETNAMPARSING_BOT_TOKEN`, `TELETHON_API_ID`, and `TELETHON_API_HASH`) configured securely before use.
- Background parsers and synchronizers run by default so Telegram listings stay current. Set the non-secret environment variable `ENABLE_BACKGROUND_JOBS=0` only for an intentional read-only diagnostic run.